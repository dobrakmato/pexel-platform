 ...
 * Copyright (C) All original Bukkit & Spigot contributors
 * 
...
public enum Material {
     ...
     * <p>
     * This is a match lookup; names will be converted to uppercase, then stripped of special characters in an attempt
     * to format it like the enum.
     * <p>
    ...
    private static <T> T[] Arrays_copyOfRange(final T[] original, final int start,
            final int end) {
        if ((original.length >= start) && (0 <= start)) {
            if (start <= end) {
                int length = end - start;
                int copyLength = Math.min(length, original.length - start);
                Object[] copy = (Object[]) Array.newInstance(original.getClass()
    ...
    public boolean isSolid() {
        if (!this.isBlock() || this.id == 0) { return false; }
        switch (this) {
            case STONE:
            case GRASS:
            case DIRT:
